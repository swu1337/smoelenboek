...
class SecretaresseModel {
    ...
    public function wijzigAfdeling() {
           $tekst_ao =  filter_input(INPUT_POST,'ao');
           $tekst_ib = filter_input(INPUT_POST,'ib');

           if(!empty($tekst_ao) || !empty($tekst_ib)) {
               $sql = "UPDATE `afdelingen` SET `omschrijving` = ( CASE WHEN `afkorting` = 'ao' THEN :tekst_ao WHEN `afkorting` = 'ib' THEN :tekst_ib END ) WHERE `afkorting` in ('ao', 'ib')";

               $stmnt = $this->db->prepare($sql);
               $stmnt->bindParam(':tekst_ao', $tekst_ao);
               $stmnt->bindParam(':tekst_ib', $tekst_ib);
    ...
    public function wijzigAnw($isDirecteur) {
        $gebruikersnaam= filter_input(INPUT_POST, 'gn');
        $voorletter=filter_input(INPUT_POST, 'vl');
        $tussenvoegsel=filter_input(INPUT_POST, 'tv');
        $achternaam=filter_input(INPUT_POST, 'an');
    ...
    public function getMedewerker() {
        ...
        if(!empty($id) && filter_var($id, FILTER_VALIDATE_INT)) {
            ...
            $stmnt->bindParam(':id', $id);
            $stmnt->execute();
            return $stmnt->fetchAll(\PDO::FETCH_CLASS,__NAMESPACE__.'\db\Contact')[0];
        } else {
            return false;
        }
    }

    public function updateAnwMedewerker() {
        ...
        if(empty($voorletter) || empty($achternaam) || empty($email)) {
            return REQUEST_FAILURE_DATA_INCOMPLETE;
        }
        ...
        $gebruiker_id = $this->getMedewerker()->getId();
    ...
    public function search() {
        ...
        if(!empty($keyword)) {
            $sql = "SELECT * FROM `contacten`  WHERE `contacten`.`achternaam` LIKE CONCAT('%', :keyword, '%');";
            $stmnt = $this->db->prepare($sql);
            ...
            if($stmnt->rowCount() > 0) {
                return $stmnt->fetchAll(\PDO::FETCH_CLASS,__NAMESPACE__.'\db\Contact');
